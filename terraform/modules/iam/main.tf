...
# AWS auth ConfigMap (adds the IAM user to the EKS cluster)
# Import the existing ConfigMap and manage only the data we need
    ...
    mapUsers = yamlencode([
      {
        ...
        groups   = ["${var.project_name}-developer-readonly"]
      }
    ])
  }
